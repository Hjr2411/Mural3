/****************************************************
 * MuralSys • app.js (V1.7.2)
 * - SEM auto-login (sempre inicia no login)
 * - Feed: texto | imagem (URL) | enquete
 * - Admin: cria/edita/exclui posts + "Novo usuário"
 * - Interações: like, comentários, voto (1 por usuário)
 ****************************************************/
...
function toast(msg){ if(!toastEl) return; toastEl.textContent = msg; show(toastEl); setTimeout(()=>hide(toastEl),2000); }
function isoNow(){ return new Date().toISOString(); }
function asDateTime(iso){ try { return new Date(iso).toLocaleString("pt-BR"); } catch { return "—"; } }
function count(obj){ return obj ? Object.keys(obj).length : 0; }
function byOrdemAsc(a,b){ const oa=Number(a.ordem??999999), ob=Number(b.ordem??999999); return oa===ob ? (new Date(b.timestamp||0)-new Date(a.timestamp||0)) : (oa-ob); }
function escapeHtml(s){ return (s||"").toString().replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;"); }
function sanitize(s){ return (s||"").toString().replace(/<[^>]*>/g,"").trim(); }

/* Sessão (somente nome; sem auto-login) */
function restoreSessionName(){
  const saved = sessionStorage.getItem("currentUser");
  if(!saved) return null;
  try { return JSON.parse(saved)?.nome || null; } catch { return null; }
}
function saveSessionName(){ sessionStorage.setItem("currentUser", JSON.stringify({ nome: currentUser?.nome || "" })); }
function clearSession(){ sessionStorage.removeItem("currentUser"); currentUser = null; }

/* Fluxo UI */
function showLogin(){ show(loginScreen); hide(appRoot); }
function showApp(){
  hide(loginScreen); show(appRoot);
  ...
  document.querySelectorAll("[data-visible-for='admin']").forEach(el=>{
    if(currentUser?.tipo === "admin") { show(el); el.removeAttribute("hidden"); }
    else { hide(el); }
  });
...
loginForm?.addEventListener("submit", async (e)=>{
  ...
  try{
    ...
    if(!snap.exists()){ loginError.textContent = "Usuário não encontrado."; show(loginError); return; }
    const data = snap.val(); // { nome, senha, tipo }
    if(data.senha !== password){ loginError.textContent = "Senha incorreta."; show(loginError); return; }
...
composerForm?.addEventListener("change", (e)=>{
  if(e.target.name === "tipo") setComposerSection(e.target.value);
});
function setComposerSection(tipo){
  if(tipo==="texto"){ show(sectionTexto); hide(sectionImagem); hide(sectionEnquete); }
  else if(tipo==="imagem"){ hide(sectionTexto); show(sectionImagem); hide(sectionEnquete); }
  else { hide(sectionTexto); hide(sectionImagem); show(sectionEnquete); }
}
document.querySelectorAll("[data-close-modal]").forEach(btn=> btn.addEventListener("click", ()=> hide(composerModal)));
...
composerForm?.addEventListener("submit", async (e)=>{
  ...
  const postData = { titulo, tipo, ordem, autor: currentUser.nome, timestamp: isoNow() };

  if(tipo==="texto"){
    postData.conteudo = (fd.get("conteudo_texto")||"").toString();
  }else if(tipo==="imagem"){
    postData.conteudo = (fd.get("conteudo_imagem")||"").toString();
  }else{ // enquete
    const opcoes = [];
    for(let i=1;i<=5;i++){ const v=(fd.get(`enquete_opcao_${i}`)||"").toString().trim(); if(v) opcoes.push(v); }
    if(opcoes.length<2){ toast("Informe ao menos 2 opções."); return; }
    postData.enquete = { opcoes, votos:{} };
  }

  try{
    if(editingPostId){ await update(ref(db, `posts/${editingPostId}`), postData); toast("Post atualizado."); }
    else { const newRef = push(ref(db,"posts")); postData.id=newRef.key; await set(newRef, postData); toast("Post publicado."); }
    hide(composerModal);
  }catch(err){ console.error(err); toast("Erro ao salvar o post."); }
});
...
function bindFeed(){
  ...
  const off = onValue(postsRef, (snap)=>{
    const list = [];
    if(snap.exists()){ const obj=snap.val(); for(const id in obj) list.push(obj[id]); list.sort(byOrdemAsc); }
    renderFeed(list);
  }, (err)=> console.error(err));
  unsubscribePosts = ()=> off();
}
function renderFeed(posts){
  feedList.innerHTML = "";
  if(!posts || posts.length===0){ show(feedEmpty); feedList.setAttribute("aria-busy","false"); return; }
  hide(feedEmpty);
...
function buildPostCard(post){
  ...
  if(post.tipo==="texto"){
    textEl.textContent = post.conteudo||""; show(textEl);
  }else if(post.tipo==="imagem"){
    if(post.conteudo){ imgEl.src = post.conteudo; show(imgBox); }
    ...
    if(post.enquete?.votos){
      const counts={}; Object.values(post.enquete.votos).forEach(v=> counts[v]=(counts[v]||0)+1);
      pollRes.textContent = Object.keys(counts).length ? "Parciais: " + Object.entries(counts).map(([k,v])=>`${k}: ${v}`).join(" · ") : "";
      if(myVote) show(pollRes);
    }
    if(myVote){ pollOpts.querySelectorAll("input[type='radio']").forEach(inp=> inp.disabled=true); }
    show(pollFrm);
    ...
    pollFrm.addEventListener("submit", async (e)=>{
      ...
      try{ await set(ref(db,`posts/${post.id}/enquete/votos/${currentUser.nome}`), chosen.value); toast("Voto registrado."); }
      catch(err){ console.error(err); toast("Erro ao registrar voto."); }
    });
  }
  ...
  likeBtn.addEventListener("click", async ()=>{
    ...
    try{
      if(likeBtn.getAttribute("aria-pressed")==="true"){ await remove(path); likeBtn.setAttribute("aria-pressed","false"); }
      else { await set(path,true); likeBtn.setAttribute("aria-pressed","true"); }
    }catch(err){ console.error(err); toast("Erro ao registrar like."); }
  });
  ...
  if(post.comentarios){
    Object.values(post.comentarios).sort((a,b)=> new Date(a.timestamp||0)-new Date(b.timestamp||0))
      .forEach(c=>{ const li=document.createElement("li"); li.textContent=`${c.usuario}: ${c.texto}`; listEl.appendChild(li); });
  }
  ...
  frmEl.addEventListener("submit", async (e)=>{
    ...
    try{
      ...
      inpEl.value=""; toast("Comentário enviado.");
  ...
  editBtn?.addEventListener("click", ()=>{ if(currentUser?.tipo!=="admin") return; openEditPost(post); });
  delBtn?.addEventListener("click", async ()=>{
    if(currentUser?.tipo!=="admin") return;
    confirmText.textContent = "Deseja excluir este post?";
    confirmDialog.showModal();
    const res = await waitDialog(confirmDialog);
    if(res==="ok"){ try{ await remove(ref(db,`posts/${post.id}`)); toast("Post excluído."); } catch(err){ console.error(err); toast("Erro ao excluir."); } }
  });
...
function openEditPost(post){
  ...
  composerForm.querySelectorAll("input[name='tipo']").forEach(r=> r.checked=(r.value===post.tipo));
  setComposerSection(post.tipo);
  if(post.tipo==="texto"){ composerForm.querySelector("#post-conteudo-texto").value = post.conteudo||""; }
  else if(post.tipo==="imagem"){ composerForm.querySelector("#post-conteudo-imagem").value = post.conteudo||""; }
  else {
    const ops = post.enquete?.opcoes||[];
    for(let i=1;i<=5;i++){ const el = composerForm.querySelector(`[name='enquete_opcao_${i}']`); if(el) el.value = ops[i-1]||""; }
  }
}

function waitDialog(dlg){ return new Promise(resolve=> dlg.addEventListener("close", ()=> resolve(dlg.returnValue), {once:true})); }

/* Admin: criar usuário (botão + Ctrl+U) */
...
async function handleCreateUserPrompt(){
  ...
  try{
    ...
    if(snap.exists()){ if(!confirm("Usuário existe. Atualizar senha/tipo?")) return; }
    await set(path,{ nome, senha, tipo });
    toast("Usuário salvo.");
  }catch(err){ console.error(err); toast("Erro ao salvar usuário."); }
}

/* INIT — SEM auto-login */
(async function init(){
  // Se já importou base.json, mantenha seed comentado
  // await seedIfEmpty();

  const lastName = restoreSessionName();
  showLogin();                         // sempre começa no login
