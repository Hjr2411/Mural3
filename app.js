...
function setupEventListeners() {
    ...
    document.getElementById('add-section-btn').addEventListener('click', () => openModal('section'));
    ...
    document.getElementById('section-form').addEventListener('submit', handleSectionForm);
...
async function handleLogin(e) {
    ...
    try {
        ...
        if (snapshot.exists()) {
            ...
            const user = Object.entries(users).find(([key, u]) => u.nome === username && u.senha === password);

            if (user) {
                currentUser = { ...user[1], key: user[0] };
...
async function checkAndInitializeDatabase() {
    ...
    if (!snapshot.exists()) {
        console.log('Banco vazio. Configure o primeiro admin.');
    }
}

async function createFirstAdmin(username, password) {
    const newRef = push(ref(database, 'usuarios'));
    await set(newRef, {
        nome: username,
        senha: password,
        tipo: 'admin',
...
function loadSections() {
    const secRef = ref(database, 'secoes');
    onValue(secRef, snapshot => {
        ...
        if (snapshot.exists()) {
            ...
            Object.entries(data).forEach(([key, val]) => {
                ...
                div.textContent = `${val.titulo} (${val.tipo})`;
...
function loadUsers() {
    ...
    onValue(userRef, snapshot => {
        ...
        if (snapshot.exists()) {
            ...
            Object.entries(data).forEach(([key, val]) => {
                const div = document.createElement('div');
                div.textContent = `${val.nome} - ${val.tipo}`;
...
function loadLogs() {
    const logRef = ref(database, 'acessos');
    onValue(logRef, snapshot => {
        ...
        if (snapshot.exists()) {
            ...
            Object.values(data).forEach(log => {
                const div = document.createElement('div');
                div.textContent = `${log.usuario} - ${log.timestamp}`;
...
function loadSuggestions() {
    const sugRef = ref(database, 'sugestoes');
...
function loadMuralContent() {
    const muralRef = ref(database, 'secoes');
    onValue(muralRef, snapshot => {
        muralContent.innerHTML = '';
        if (snapshot.exists()) {
            const secoes = snapshot.val();
            Object.values(secoes).forEach(sec => {
                const div = document.createElement('div');
                div.textContent = `${sec.titulo}: ${sec.conteudo || ''}`;
                muralContent.appendChild(div);
            });
        }
    });
}
...
async function handleSectionForm(e) {
    ...
    const refSec = editingSectionKey
        ? ref(database, `secoes/${editingSectionKey}`)
        : push(ref(database, 'secoes'));
...
async function handleUserForm(e) {
    ...
    const refUser = editingUserKey
        ? ref(database, `usuarios/${editingUserKey}`)
